...
import { Sofa, Clock, MapPin, Plus, Eye, Sparkles, Palmtree, Gift } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { listingsService, Listing } from '../lib/listings';
import CategoryBadge from '../components/CategoryBadge';

const ForHomePage: React.FC = () => {
  const { user } = useAuth();
  ...
  const subcategories = [
    { value: null, label: 'Всички', icon: Sofa },
    { value: 'practical', label: 'Практично', icon: Sparkles },
    { value: 'decorations', label: 'Декорации', icon: Palmtree },
    { value: 'holidays', label: 'Празници', icon: Gift },
  ];

  useEffect(() => {
    const fetchListings = async () => {
      setLoading(true);
      const { listings, error } = await listingsService.getListingsByCategory('for-home', activeSubcategory || undefined);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-2xl mx-auto" itemProp="description">
            Интересни декорации за вашия дом - разгледай и поръчай!
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {subcategories.map((subcategory) => {
            ...
            const isActive = activeSubcategory === subcategory.value;
            return (
              <button
                ...
                className={`flex items-center space-x-2 px-6 py-3 rounded-xl font-medium transition-all duration-200 ${
                  isActive
                    ? 'bg-berry-700 text-white shadow-lg scale-105'
                    : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200 hover:border-berry-300'
                }`}
              >
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-6">
              Все още няма публикувани обяви за декорации за дома.
            </p>
          </div>
        )}
      </div>
