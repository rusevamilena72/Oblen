...
import { X, Mail, User, ShoppingCart, Send, AlertCircle, CheckCircle } from 'lucide-react';
...
const OrderForm: React.FC<OrderFormProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.email || !formData.orderDetails) {
    ...
    try {
      ...
      const templateParams = {
        ...
        customer_details: formData.customerDetails || 'Не са предоставени',
          ...
          {success && (
            <div className="mb-6 bg-green-50 border border-green-200 rounded-lg p-4 flex items-center space-x-2">
              ...
              <span className="text-green-700 text-sm">
                Поръчката е изпратена успешно! Ще получите потвърждение скоро.
              </span>
            </div>
          )}
